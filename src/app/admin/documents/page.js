...
export default function AdminDocumentsPage() {
    ...
    // Fetch all documents (admin view — fetches everything)
        ...
        <div className="w-full h-full flex flex-col items-center relative overflow-visible">
            ...
            <div className="w-full text-center py-6 md:py-14 mb-2 animate__animated animate__fadeIn relative flex flex-col items-center justify-center min-h-[15vh] px-4">
                <div className="relative z-10 w-full">
                    <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold mb-3 tracking-wide text-gradient-premium bg-clip-text uppercase">
                        Secure Vault
                    </h1>
                    ...
                    {isAdmin && (
                        <button
                            ...
                            className="px-6 sm:px-8 py-2.5 sm:py-3 bg-gradient-premium rounded-full text-white font-black text-[10px] sm:text-xs uppercase tracking-widest shadow-xl hover:scale-105 transition-all flex items-center gap-2 mx-auto"
                        >
            ...
            <div className="w-full max-w-4xl px-4 animate__animated animate__fadeInUp relative z-10 space-y-10">
                {isLoading ? (
                    ...
                    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-12 text-center">
                        <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">No documents in the vault</p>
                    </div>
                ) : (
                    categories.map((category, gIdx) => (
                        <div key={gIdx} className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                            <div className="bg-gray-50 px-5 sm:px-8 py-4 sm:py-5 border-b border-gray-100 flex items-center gap-3 sm:gap-4">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 sm:w-5 h-5 text-[#4A4A4A]">
                                        ...
                                        <div key={dIdx} className="p-4 sm:p-6 flex items-center justify-between hover:bg-gray-50 transition-colors group gap-4">
                                            <div className="flex items-center gap-3 sm:gap-5 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    <p className={`text-xs sm:text-sm font-bold tracking-tight transition-colors truncate ${isViewed ? 'text-gray-300' : 'text-gray-900 group-hover:text-[#153A6A]'}`}>{doc.name}</p>
                                                ...
                                                <button
                                                    ...
                                                    className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg font-black text-[9px] sm:text-[10px] uppercase tracking-widest transition-all
                                                        ${isViewed
                                                            ...
                                                            : 'bg-white text-black border-2 border-gray-100 hover:border-[#4A4A4A] hover:shadow-md'}`}
                                                >
                                                    {isViewed ? 'Seen' : 'View'}
                                                </button>
