...
export default function CalendarPage({ isAdmin = false }) {
    ...
    const handleAddEvent = async (e) => {
        ...
        if (!newEvent.userId) {
            Swal.fire({ title: 'User Required', text: 'Please select a user to assign this schedule to.', icon: 'warning', confirmButtonColor: '#4A4A4A' });
            return;
        }
        ...
        try {
            ...
            Swal.fire({ title: 'Error', text: 'Failed to save schedule. Please try again.', icon: 'error', confirmButtonColor: '#4A4A4A' });
        }
    };
        ...
        <>
            <div className="w-full h-full space-y-4 md:space-y-8 animate__animated animate__fadeIn relative overflow-visible">
                ...
                <div className="w-full text-center py-6 md:py-10 relative flex flex-col items-center justify-center px-4">
                    <div className="relative z-10 w-full">
                        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-black mb-3 tracking-tight text-gradient-premium bg-clip-text uppercase leading-none text-center">
                            Calendar
                        </h1>
                            ...
                            <div className="bg-white rounded-[13px] h-full shadow-[inset_0_2px_10px_rgba(255,255,255,0.9),inset_0_-2px_10px_rgba(0,0,0,0.02)] flex flex-col relative overflow-hidden">
                                ...
                                {upcomingSchedules.length === 0 ? (
                                    ...
                                    upcomingSchedules.map(item => (
                                        <div key={item.id} className="p-3 border border-gray-50 rounded-xl hover:border-[#4A4A4A]/30 hover:bg-gray-50/50 transition-all group">
                                            <div className="flex items-center justify-between mb-1">
                                                ...
                                                <span className="text-[10px] text-[#4A4A4A] font-black">{item.time}</span>
                                            </div>
                                            <h4 className="text-sm font-bold text-black group-hover:text-[#4A4A4A] transition-colors line-clamp-1">{item.title}</h4>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate__animated animate__fadeIn">
                    <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] shadow-2xl border-2 border-[#4A4A4A]/50 w-full max-w-md overflow-hidden animate__animated animate__zoomIn">
                        <div className="bg-gradient-premium py-4 px-6 flex items-center justify-between border-b border-[#1A1A1A]/30">
                        ...
                        <form onSubmit={handleAddEvent} className="p-6 sm:p-8 space-y-5 md:space-y-6">
                            ...
                            <div ref={userDropdownRef} className="relative space-y-1.5 md:space-y-2">
                                <label className="block text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-[#153A6A] mb-2">Assign to User</label>
                                <div
                                    className="w-full bg-gray-50 border-2 border-gray-100 rounded-xl px-4 py-2.5 sm:py-3 text-xs sm:text-sm font-bold text-black cursor-pointer flex items-center justify-between hover:border-[#4A4A4A]/50 transition-all shadow-sm"
                                ...
                                {showUserDropdown && (
                                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border-2 border-[#4A4A4A]/30 rounded-xl shadow-2xl z-50 overflow-hidden">
                                        <div className="p-2 border-b border-gray-100">
                                            <input
                                                autoFocus
                                                type="text"
                                                placeholder="Search users..."
                                                className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-[10px] font-bold text-black outline-none focus:border-[#4A4A4A] transition-all"
                                                value={userSearch}
                                            ...
                                            {filteredUsers.length === 0 ? (
                                                ...
                                                filteredUsers.map(u => {
                                                    ...
                                                    return (
                                                        <div
                                                            key={uid}
                                                            className="px-4 py-3 flex items-center gap-3 hover:bg-[#4A4A4A]/10 cursor-pointer transition-colors"
                                                            onClick={() => {
                                                        ...
                                                        >
                                                            <div className="w-7 h-7 rounded-full bg-gradient-premium flex items-center justify-center text-black font-black text-[9px] flex-shrink-0">
                            ...
                            <div className="space-y-1.5 md:space-y-2">
                                <label className="block text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-[#153A6A] mb-2">Description</label>
                                <textarea
                                    rows={2}
