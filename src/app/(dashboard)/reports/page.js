...
const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        return (
            <div className="bg-white/90 backdrop-blur-sm p-3 border-2 border-[#4A4A4A] rounded-lg shadow-xl">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
                            <div>
                                <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-gray-950 leading-tight uppercase tracking-tight">
                                    {viewMode === 'monthly'
                                        ? <>Allocation <span className="text-gradient-premium">{selectedYear}</span></>
                                        : <>Yearly <span className="text-gradient-premium">Overview</span></>
                                    }
                                </h2>
